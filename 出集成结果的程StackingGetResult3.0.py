...
inbasePath = r'D:\\Stacking_okData\\teststd\\ToASCII\\' 
outbasePath = r"D:\\Stacking_okData\\teststd\\Result3\\"

prgbasePath = r"D:\\Stacking_okData\\teststd\\prj\\" 

#这是存放一个.prj后缀文件的文件夹

filepath = 'D:\\Stacking_okData\\teststd\\SHmean9.csv'

pd1 = pd.read_csv(filepath_or_buffer = filepath)
filelist = pd1.columns[2:-1]
...
rf_model = RandomForestClassifier(n_estimators = 10,min_samples_split = 100,min_samples_leaf =10,max_depth = 1,max_features= 'sqrt',random_state = 0)
gdbc_model = GradientBoostingClassifier(n_estimators = 80,max_depth = 3,min_samples_split=100,min_samples_leaf = 20,max_features = 'sqrt',subsample = 0.7,learning_rate = 0.05,random_state = 10)
knn_model = KNeighborsClassifier(n_neighbors = 39,weights = 'uniform')
xgb_model = xgb.XGBClassifier(learning_rate =0.1,n_estimators=150,max_depth=4,min_child_weight=3,seed =0,subsample =0.9,colsample_bytree = 0.8,gamma = 0.1,reg_alpha = 0.05,reg_lambda =0.1)
svc_model = SVC( C= 10, kernel = 'linear')
...
train_x, test_x, train_y, test_y  = loadData(filepath)
